...
if ( $#ARGV != 2 ) {
	print
"Usage: perl parse-list-of-political-authors.pl <ExtracedListsXml> <Query> <OutputName>\n";
	exit 1;
}

print "Parsing List of political authors now ... ";
...
if ( $xml =~ /(.*)List of political authors(.*?)<\/page>/s ) {
	my $list = $2;

	# Select the part from <text to == See also ==.
	if ( $list =~ /(.*)\<text(.*?)== See also ==(.*)/s ) {
		my $current = $2;
...
# Always select the wikipedia link directly after a * (plus possible non-word chars)
		...
		print "Failed to find an area between <text and == See also ==!\n";
	}
