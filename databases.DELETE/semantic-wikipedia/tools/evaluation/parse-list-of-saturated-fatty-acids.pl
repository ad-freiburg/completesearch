...
if ( $#ARGV != 2 ) {
	print
"Usage: perl parse-list-of-saturated-fatty-acids.pl <ExtracedListsXml> <Query> <OutputName>\n";
	exit 1;
}

print "Parsing List of saturated fatty acids ... ";
...
if ( $xml =~ /\<title\>List of saturated fatty acids(.*?)<\/page>/s ) {
	my $list = $1;

	# Select the part from "wikitable" to "==See also==".
	if ( $list =~ /(.*)wikitable(.*?)==See also==(.*)/s )
	{
		my $current = $2;

  # Always select the first wikipedia link of a row
		while ( $current =~ /\|\-(.*?)\[\[(.*?)[\|\]]/gs ) {
		...
		print "Failed to find an area between wikitable and ==See also==!\n";
	}
