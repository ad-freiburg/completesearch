...
class ConvertAsciiToBinary : public testing::Test
{
 public:
 ...
 protected:
  ...
  : _base("ConvertAsciiToBinary.TMP"),
...
TEST_F(ConvertAsciiToBinary, ConvertAsciiToBinary)
{
  // Create unsorted words File with this string

  string inData = "wort3\t1\t1\t1\n"
          "wort1\t1\t1\t3\n"
          "wort1\t1\t1\t2\n"
          "ww\t1\t1\t3\n"
          "wort0\t1\t1\t1\n";

  writeStringToFile(inData, _base + _extensionWords_unsorted);

  // Execute CovertAsciiToBinary with created File

  string sysLine = string("./ConvertAsciiToBinary ")
          + _base + _extensionWords_unsorted;
  ASSERT_EQ(system(sysLine.c_str()), 0);

  // Testing of created Vocabulary
  string vocabulary = "wort0\n"
          "wort1\n"
          "wort3\n"
          "ww\n";
  compareFileWithString(vocabulary, _base + _extensionVocabulary);
  ...
  compareBinaryFileWithVector(expVector, _base + _extensionBinary_unsorted);
  removeFile(_base + _extensionVocabulary);
  removeFile(_base + _extensionBinary_unsorted);
  removeFile(_base + _extensionWords_unsorted);
}
